...
Consider \texttt{IOUtils} from Apache Commons IO, a 4,012-line class with 172
methods and 8 fields. Its $\text{LCOM5}=1.0$ and $\text{TCC}=0.064$, so
metric-only tools propose zero viable extractions. Yet manual inspection reveals
distinct responsibilities buried among the overloaded utility methods.

GenEC's analysis discovered that the resource-loading methods
(\texttt{resourceToString()}, \texttt{resourceToURL()} and their overloads) form
a cohesive cluster with internal cohesion of 0.80 and external coupling of 0.25.
The LLM named it ``ResourceLoader'' with the rationale: ``These methods form a
cohesive unit responsible for loading classpath resources and converting them to
...
\begin{itemize}
  ...
  \item \textbf{Verified suggestions extract 21.8\% of methods} on average
        across 21 classes with verified output, with per-class coverage ranging
        from 2.3\% to 78.3\%.
...
\label{eq:static}
\end{equation}
where $\text{call\_count}(m_i,m_j)$ is the number of times method $m_i$ calls
$m_j$, $\text{shared\_fields}(m_i,m_j)$ is the number of class fields accessed
by both methods, $\alpha_{\text{call}}=1.0$, $\alpha_{\text{field}}=0.8$, with
an additional $\alpha_{\text{shared\_field}}=0.9$ for shared field access
...
\begin{equation}
G_{\text{fused}} = \alpha \cdot \text{normalize}(G_{\text{static}})
  + (1 - \alpha) \cdot \text{normalize}(G_{\text{evo}})
\label{eq:fusion}
\end{equation}
...
clusters).  Multi-resolution search over $\{0.5,0.75,1.0,1.25,1.5\}$ is
...
\midrule
Evolutionary coupling & 30 & strong ($>$0.5)=30, moderate ($>$0.3)=20, weak ($>$0)=10, none=5 \\
Internal cohesion & 35 & high ($>$0.7)=35, good ($>$0.5)=25, moderate ($>$0.3)=15, low=5 \\
External coupling (inv.) & 25 & low ($<$0.3)=25, moderate ($<$0.5)=15, high ($<$0.7)=5, very high=0 \\
...
T2: Structural  & Member completeness, dep.\ satisfaction, ref.\ consistency & Reject with explanation \\
...
% ================================================================
\section{Motivating Example: Apache Commons IOUtils}
\label{sec:motivating}

To illustrate GenEC's approach concretely, we walk through its application to
\texttt{IOUtils} from Apache Commons IO, a widely used utility class that
...
\texttt{IOUtils} is a 4,012-line utility class with 172 methods providing I/O
convenience functions. It has been part of Apache Commons IO since 2002 and is
used by thousands of downstream projects.

\begin{table}[h]
\centering\small
\caption{IOUtils class statistics.}
\label{tab:ioutils-stats}
\begin{tabular}{@{}lr@{}}
\toprule
...
\begin{lstlisting}[style=javacode,caption={Representative subset of IOUtils (172 methods).},label=lst:ioutils]
public class IOUtils {
    ...
    // Resource loading (the cluster GenEC
    // extracts)
...
JDeodorant and similar metric-based tools analyze \texttt{IOUtils} and find:
\begin{itemize}
  \item \textbf{High field sharing:}  Many methods access common fields (e.g.,
        \texttt{DEFAULT\_BUFFER\_SIZE}, \texttt{SKIP\_BUFFER}).
  \item \textbf{Extensive method calls:}  Utility methods call each other
        frequently.
  \item $\text{LCOM5}=1.0$, $\text{TCC}=0.064$: The class is large and
        uncohesive by metrics, yet the field-sharing baseline produces only 1
        suggestion (a generic ``IOUtils\$Helper1'' grouping 13 methods by shared
        field access).
\end{itemize}

\textbf{Result:}  The field-sharing baseline produces one low-quality cluster
with no verification. GenEC's fused analysis finds 37 candidate clusters,
filters down to 1 verified extraction.

The challenge with \texttt{IOUtils} is that its 172 methods are mostly static
utilities with minimal field sharing, making structural clustering difficult.
Most methods are independent overloaded variants that do not share fields or
call each other.
...
graph over \texttt{IOUtils'} 172 methods. With such a large utility class, most
methods are static and share few fields, producing a sparse graph.

\textit{Stage~2: Evolutionary Coupling.}  Git history mining reveals co-change
patterns among the resource-loading methods (\texttt{resourceToString},
\texttt{resourceToURL} and their overloads), which changed together across
commits while rarely changing with stream-copying or encoding methods.

\textit{Stage~3: Fused Graph and Clustering.}  Community detection on the fused
graph produces 37 candidate clusters. After quality filtering (internal cohesion
$> 0.35$, size constraints), only 1 cluster passes: a group of 4
resource-loading methods with internal cohesion of 0.80 and external coupling of
0.25.

\textit{Stage~4: LLM Naming.}  The LLM names the cluster ``ResourceLoader''
with the rationale: ``These methods form a cohesive unit responsible for loading
classpath resources and converting them to strings. They handle the complete
workflow from resource discovery to content conversion.''  Confidence: 0.90.

% ── 4.4 ──
\subsection{Verification Results}
\label{sec:verif-results}

The single candidate extraction passes through the multi-tier verification
pipeline.

\begin{table}[h]
\centering\small
\caption{Verification results for IOUtils candidate extractions.}
\label{tab:ioutils-verify}
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Cluster} & \textbf{T1: Compile} & \textbf{T2: Structural} & \textbf{T3: Behavioral} & \textbf{Result} \\
\midrule
...
GenEC produces one verified suggestion for \texttt{IOUtils}:

\textbf{Verified Suggestion: ResourceLoader.}
\begin{itemize}
  \item \textbf{Methods:} \texttt{resourceToString(String,Charset)},
        \texttt{resourceToString(String,Charset,ClassLoader)},
        \texttt{resourceToURL(String)},
        \texttt{resourceToURL(String,ClassLoader)}.
  \item \textbf{Fields:}  None (stateless extraction).
  \item \textbf{Quality tier:}  ``should'' (score 75.0).
  \item \textbf{LLM confidence:}  0.90.
  \item \textbf{Post-extraction LCOM5:}  1.0 (original class barely
        changes, since only 4 of 172 methods were extracted).
\end{itemize}
...
\begin{enumerate}
  \item \textbf{Large utility classes are hard:}  With 172 methods and minimal
        field sharing, \texttt{IOUtils} yields only 1 verified extraction out of
        37 candidates, a 2.7\% filter pass rate.
  \item \textbf{Quality over quantity:}  The single verified suggestion has
        high internal cohesion (0.80) and reasonable coupling (0.25), making it
        a genuinely useful extraction.
  \item \textbf{LLM adds semantic value:}  ``ResourceLoader'' is far more
        meaningful than ``IOUtils\$Helper1'' (the field-sharing baseline's
        name).
  \item \textbf{Honest limitations:}  \texttt{IOUtils'} design (172 static
        utility methods with minimal interdependencies) resists decomposition.
        GenEC correctly identifies this by rejecting 36 of 37 clusters.
\end{enumerate}
...
\begin{tabular}{@{}lrrrr@{}}
\toprule
\textbf{Project} & \textbf{Classes} & \textbf{GenEC Sugg.} & \textbf{Verified} & \textbf{Baseline} \\
\midrule
...
\midrule
\textbf{Total}     & \textbf{23} & \textbf{178} & \textbf{104} & \textbf{37} \\
\bottomrule
\end{tabular}
\end{table}
...
GenEC extracted a total of 615 methods out of 2,823 original methods (21.8\%
overall). Per-class coverage ranged from 2.3\% (\texttt{IOUtils}, 4 of 172
methods) to 78.3\% (\texttt{ChartPanel}, 90 of 115 methods), with a mean of
30.3\%. Five classes exceeded 50\% coverage: \texttt{ChartPanel} (78.3\%),
\texttt{Dfp} (57.5\%), \texttt{AccurateMath} (56.8\%), \texttt{FilenameUtils}
(54.2\%), and \texttt{DateUtils} (52.2\%). These results show that GenEC
...
HyperbolicFunctions & Hyperbolic math ops (sinh, cosh, tanh) & AccurateMath\$Helper1 \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{table}[t]
\centering\small
\caption{Per-class latency and suggestion counts (selected classes).}
\label{tab:rq4}
\begin{tabular}{@{}llrrrr@{}}
\toprule
\textbf{Class} & \textbf{Project} & \textbf{Meth.} & \textbf{Time(s)} & \textbf{Sugg.} & \textbf{Verif.} \\
\midrule
...
\label{tab:ablation}
\begin{tabular}{@{}lp{2.4cm}rrrr@{}}
\toprule
\textbf{Variant} & \textbf{Description} & \textbf{Clust.} & \textbf{Verif.\%} & \textbf{Coh.} & \textbf{Time} \\
\midrule
full             & Complete pipeline       & 68.6 & 10.3\% & 0.171 & 138.4 \\
no\_evo          & No evolutionary coupling & 53.9 & 11.2\% & 0.245 & 79.4  \\
no\_llm          & No LLM naming/filtering & 68.1 & 10.3\% & 0.136 & 107.4 \\
no\_verification & No compilation verif.   & 68.4 &  7.2\% & 0.124 &  63.9 \\
high\_static     & Higher static ($\alpha{=}0.95$) & 68.0 & 8.3\% & 0.129 & 106.0 \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Refactoring    & Extract Method   & Code changes     & Extract Class \\
LLM role       & Suggests fragments & Generates variants & Names + rationales only \\
IDE engine     & IntelliJ IDEA    & TBE + IDE        & Eclipse JDT \\
Verification   & IDE + tests      & Tests + PR review & Multi-tier \\
Complexity     & Single method    & Single transform  & Multiple coordinated methods \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Capability} & \textbf{JDeodorant} & \textbf{HECS} & \textbf{EM-Assist} & \textbf{GenEC} \\
\midrule
...
\begin{enumerate}
  ...
  \item \textbf{Most rejections happen at compilation, not at deeper tiers.}
        The 41.6\% rejection rate is dominated by code-generation failures and
        missing dependencies rather than subtle semantic bugs. This points to
        JDT integration as the primary bottleneck for future improvement.
\end{enumerate}
