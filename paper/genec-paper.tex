...
structural edits to deterministic code generation built on Eclipse JDT's AST rewriting infrastructure. Every
...
validation. Clusters rejected during pre-extraction validation are not silently discarded but
...
while blocking 41.6\% of proposals that would have broken compilation,
structural integrity, or test behavior. On the HECS benchmark, GenEC achieves macro $F_1=0.478$ on the 21
...
suggestions in concrete data. All code generation is delegated to a deterministic engine built on Eclipse JDT's AST rewriting APIs---parsing, node construction, and reference resolution are handled by JDT's DOM infrastructure, ensuring mechanical correctness without relying on LLM-generated code.
...
\begin{enumerate}
  ...
  \item \textbf{Behavioral tests} when available, validating that existing test suites pass against the refactored code.
\end{enumerate}
Clusters rejected during pre-extraction validation are not silently discarded. Instead, GenEC
...
developers can address the issue manually if desired. Suggestions that fail post-extraction verification tiers are rejected with detailed explanations logged for debugging.
...
\begin{itemize}
  ...
  \item \textbf{Multi-tier verification blocked 41.6\%} of proposed
        extractions that would have broken compilation, structural integrity, or test behavior,
...
\begin{enumerate}
  \item \textbf{A hybrid Extract Class technique} that fuses static dependency
        ...
        structural edits to deterministic code generation built on Eclipse JDT's
        AST rewriting infrastructure (ASTParser, ASTRewrite, ListRewrite),
        extending the ``LLM for insight, IDE for correctness'' approach~\cite{Pomian2024EMAssist,Dilhara2024PyCraft}
...
The LLM never sees or generates code. All structural edits are performed by
deterministic code generation built on Eclipse JDT's AST rewriting
infrastructure (ASTParser, ASTRewrite, ListRewrite), not a built-in
refactoring processor. This architecture preserves the LLM's semantic
intelligence while eliminating its mechanical failures.
...
\begin{enumerate}
  ...
  \item \textbf{Tier~2 (Structural Integrity):}  Verify member completeness,
        ...
        extracted classes.
  \item \textbf{Tier~3 (Behavioral):}  Validate that existing test suites
        pass after extraction.
\end{enumerate}
Suggestions that fail any tier are rejected before developers see them.
Pre-extraction validation generates structural transformation plans that
provide actionable guidance when automatic extraction is not safe.
...
The pipeline consists of six sequential stages (shown in
...
the LLM (Claude Sonnet, temperature~0.3) with a structured request that
includes:
\begin{enumerate}
...
\textit{Stage~5: Refactoring Execution.}  Deterministic code generation built
on Eclipse JDT's AST rewriting infrastructure (ASTParser, ASTRewrite,
ListRewrite) performs the actual Extract Class refactoring:
\begin{enumerate}
...
\end{enumerate}
JDT's AST rewriting handles visibility adjustments (private $\to$
...
\midrule
T1: Compilation & Compile with stubs & Reject \\
...
Verification produces one of two outcomes: (1)~\textbf{Pass:} suggestion is
verified and ready for developer review; (2)~\textbf{Reject:} suggestion is
discarded (logged for debugging). Structural transformation plans are generated
separately during pre-extraction validation (before verification) when a
cluster cannot be safely extracted.
...
Code generation & JDT AST rewriting & Deterministic, correct \\
Ref.\ updates & JDT AST rewriting & Full source analysis \\
\bottomrule
\end{tabularx}
\end{table}
...
An equivalence checker exists in the codebase but is disabled by default;
behavioral validation relies on Tier~3 test-suite execution.
...
\textit{Structural Transformation Plans.}  During pre-extraction validation,
when a cluster cannot be safely extracted, GenEC generates actionable plans:

\begin{lstlisting}[style=textblock]
Pre-extraction validation rejected {cluster}.
...
Before presenting results, we define key terms used throughout the evaluation
to ensure consistent interpretation:

\begin{description}
  \item[Raw clusters] All communities detected in the fused dependency graph by
    the Leiden algorithm, before any filtering.
  \item[Candidate suggestions] Clusters that survive size constraints (3--30
    members), minimum cohesion threshold (0.35), and deduplication. Each
    candidate is assigned a quality tier (``should,'' ``could,'' or
    ``potential'') based on internal cohesion and external coupling scores.
  \item[Verified suggestions] Candidates that pass all applicable verification
    tiers: Tier~1 (compilation with stub generation), Tier~2 (structural
    integrity), and Tier~3 (behavioral test validation, when tests are
    available).
  \item[Verification rate] The ratio of verified suggestions to candidate
    suggestions (i.e., \#verified / \#candidates).
  \item[Structural transformation plans] Actionable guidance documents generated
    for clusters rejected during pre-extraction validation (before code
    generation), describing the impediment and suggesting manual remediation
    steps.
\end{description}
...
interpretability compared to baseline naming?
...
\textbf{Metrics.}  Banned-suffix compliance rate; mean word count per name;
qualitative inspection of semantic labels. A controlled developer study with
Likert-scale ratings is planned as future work
...
This evaluation scale (23 live classes from 6 projects, 92 benchmark
instances from 18 projects) exceeds prior Extract Class evaluations:
JDeodorant~\cite{Fokaefs2011JDeodorant} evaluated on 3 projects,
Bavota et al.~\cite{Bavota2013EMSE} on 5 projects, and
HECS~\cite{Cui2024HECS} on a comparable benchmark of 92 instances.
...
\textbf{Graceful degradation on static-only instances.}  On the 71 instances
without meaningful Git history, GenEC automatically detects the shallow
repository and falls back to structural-only analysis ($\alpha$ raised to
0.95). The resulting $F_1{=}0.076$ reflects the difficulty of the task when
only static signals are available, not a failure of the tool. This
demonstrates GenEC's adaptive behavior: rather than producing unreliable
suggestions from missing evolutionary data, it degrades to a conservative
mode that relies on structural cohesion alone. The low $F_1$ on static-only
instances also highlights the value of evolutionary coupling when it IS
available ($F_1{=}0.478$, a $6.3\times$ improvement).
...
\textbf{Sample size.}  Our live evaluation covers 23 God Classes, and our
HECS benchmark evaluation covers 92 instances (21 with full evolutionary
context). While the total of 115 evaluated instances across 24 projects
exceeds most prior Extract Class evaluations (JDeodorant evaluated on 3
projects, Bavota on 5), we acknowledge that the 21-instance subset with
evolutionary coupling is small for robust statistical generalization. We
mitigate this through: (1)~a controlled ablation showing identical $F_1$
with and without evo on the same 21 instances, (2)~a separate 8-class
ablation on our live benchmark, and (3)~bootstrap confidence intervals on
all key metrics. Expanding the HECS evaluation to more instances with full
Git history is straightforward with the provided benchmark adapter script.
...
\textbf{Human evaluation.}  RQ3 (naming quality) relies on the authors'
qualitative assessment of 178 suggestion names rather than independent
developer ratings. This is a significant limitation. We provide quantitative
proxy metrics (banned-suffix compliance, word count, confidence scores) but
acknowledge these do not substitute for developer judgment. A formal developer
study following the EM-Assist protocol~\cite{Pomian2024EMAssist} (15--20
professional developers, Likert-scale ratings, within-subjects design) is our
immediate next step. The study materials, survey instrument, and analysis
scripts are included in the replication package.
...
per project. The LLM (Claude Sonnet, temperature~0.3) introduces
...
\textbf{Tornhill and Borg}~\cite{cassee2024refuctoring} studied the impact of
...
IDE engine     & IntelliJ        & TBE + IDE        & JDT AST rewriting \\
...
\textbf{GenEC's contribution.}  To our knowledge, we are the first to combine evolutionary
...
% ================================================================
\section{Ethics, AI Disclosure, and Deployment Considerations}
\label{sec:ethics}

\textbf{Generative AI disclosure.}  In compliance with ACM and IEEE policies on
generative AI in scientific publishing, we disclose the following: GenEC uses
Anthropic's Claude Sonnet (model ID: \texttt{claude-sonnet-4-20250514}) as a
pipeline component for generating class names, rationales, and confidence
scores. The LLM is not listed as an author and did not contribute to the writing
of this manuscript. All LLM outputs used in the evaluation are cached and
included in the replication package to ensure reproducibility independent of API
availability.

\textbf{Data sent to the LLM.}  For each candidate cluster, GenEC transmits:
(1)~method signatures and field declarations (not full method bodies),
(2)~co-change frequency counts, and (3)~dependency edge weights. No proprietary
source code, credentials, or personally identifiable information is sent. For
projects with confidentiality constraints, GenEC supports configuration of
local/on-premise LLM endpoints as a drop-in replacement.

\textbf{Reproducibility.}  To mitigate non-determinism in LLM responses, we pin
the model version, set temperature to 0.3, and provide a deterministic ``replay
mode'' that reads cached responses for exact reproduction of reported results.
To avoid data contamination from repository version mismatch, all subject
repositories are pinned to specific commit SHAs recorded in the replication
package; the HECS benchmark instances use the exact parent commit of each
ground-truth refactoring.
The replication package includes all prompts, cached outputs, evaluation scripts,
and a Docker image for environment reproducibility.

\textbf{Open-source commitment.}  GenEC's complete source code, VS~Code
extension, evaluation harness, and datasets are publicly available under an
open-source license at our GitHub
repository~\cite{Tomar2025GenEC}.
...
LLM usage to semantic artifacts while delegating code generation to deterministic
AST rewriting built on Eclipse JDT's infrastructure, GenEC produces verified,
compilable, and explainable refactoring suggestions.
...
The key lessons are:
\begin{enumerate}
  \item \textbf{Evolutionary coupling improves candidate discovery} (+27.4\%
        more clusters) rather than member-selection accuracy (identical $F_1$ in
        controlled ablation), particularly valuable in utility classes where
        shared infrastructure masks conceptual boundaries.
  \item \textbf{Constraining LLMs to semantics} eliminates the 40--76\%
        hallucination rate observed in unconstrained LLM
        refactoring~\cite{cassee2024refuctoring, Liu2024Empirical} while
        preserving the naming and explanation quality that helps developers
        decide whether to accept a suggestion.
  \item \textbf{Multi-tier verification is essential} for developer
        trust---41.6\% of proposed extractions were blocked across compilation,
        structural integrity, and behavioral tiers before reaching developers.
  \item \textbf{Structural transformation plans} turn failures into actionable
        guidance, making the tool useful even when automatic extraction is not
        safe.
\end{enumerate}

\textbf{Future work.}  Our most pressing priority is a controlled developer
study following the protocol established by
EM-Assist~\cite{Pomian2024EMAssist}: we will recruit 15--20 professional Java
developers to rate GenEC's suggestions on name appropriateness, rationale
clarity, and overall acceptance using Likert-scale surveys in a
within-subjects design. The study materials, survey instrument, and analysis
scripts are already included in the replication package. This evaluation is
essential to address the current absence of human-centric validation and to
establish whether GenEC's mechanically verified suggestions translate into
developer trust and adoption.  Beyond this immediate step, we plan to extend
GenEC to support incremental God Class decomposition (multiple sequential
extraction passes), cross-file Extract Class refactoring, and integration
with IntelliJ IDEA's refactoring infrastructure alongside Eclipse JDT\@. We also
plan to evaluate on larger industrial codebases and submit GenEC-generated
pull requests to the subject projects to measure real-world acceptance
rates, following the methodology of PyCraft~\cite{Dilhara2024PyCraft}.
