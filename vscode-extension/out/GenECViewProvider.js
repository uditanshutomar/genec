...
exports.GenECViewProvider = void 0;
const vscode = require("vscode");
const cp = require("child_process");
const path = require("path");
const fs = require("fs");
class GenECViewProvider {
        ...
        webviewView.webview.onDidReceiveMessage((data) => __awaiter(this, void 0, void 0, function* () {
            switch (data.type) {
                case 'refactor': {
                    ...
                    const minCohesion = config.get('clustering.minCohesion');
                    // Sync API Key to settings if provided
                    if (data.apiKey) {
                        yield config.update('apiKey', data.apiKey, vscode.ConfigurationTarget.Global);
                    }
    ...
    _sendSettings() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const settings = {
                ...
                minClusterSize: config.get('clustering.minClusterSize'),
    ...
    _saveSettings(settings) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!this._view)
                return;
            const config = vscode.workspace.getConfiguration('genec');
            yield config.update('pythonPath', settings.pythonPath, vscode.ConfigurationTarget.Global);
            yield config.update('apiKey', settings.apiKey, vscode.ConfigurationTarget.Global);
            yield config.update('autoApply', settings.autoApply, vscode.ConfigurationTarget.Global);
    ...
    _applyRefactoring(index) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                // Open the new file
                const doc = yield vscode.workspace.openTextDocument(newClassPath);
    ...
    _runRefactoring(pythonPath, finalApiKey, autoApply, minClusterSize, maxClusterSize, minCohesion) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a, _b;
            if (!this._view) {
                return;
            ...
            if (!finalApiKey) {
                let storedApiKey = config.get('apiKey') || process.env.ANTHROPIC_API_KEY;
                if (!storedApiKey) {
                    storedApiKey = yield vscode.window.showInputBox({
                        prompt: 'Enter your Anthropic API Key',
                        password: true,
                        ignoreFocusOut: true
                    });
                    if (storedApiKey) {
                        yield config.update('apiKey', storedApiKey, vscode.ConfigurationTarget.Global);
                    }
                }
            ...
            try {
                ...
                const output = yield new Promise((resolve, reject) => {
                    ...
                    (_b = this._currentProcess.stderr) === null || _b === void 0 ? void 0 : _b.on('data', (data) => {
                        const dataStr = data.toString();
                        stderr += dataStr;
                ...
                try {
                    ...
                    if (result.applied_refactorings && result.applied_refactorings.length > 0) {
                        ...
                        if (successfulApps.length > 0) {
                            ...
                            const appliedNames = new Set(successfulApps.map((app) => {
                                const filePath = app.new_class_path || app.new_class;
                                return path.basename(filePath).replace('.java', '');
                            }));
                            const notApplied = allSuggestions.filter((s) => !appliedNames.has(s.name));
                            const couldSuggestions = notApplied.filter((s) => s.quality_tier === 'could');
                            const potentialSuggestions = notApplied.filter((s) => s.quality_tier === 'potential');
                            ...
                            if (couldSuggestions.length > 0) {
                                ...
                                summaryHtml += `<ul style="margin: 5px 0; padding-left: 20px;">`;
                                summaryHtml += couldSuggestions.map((s) => {
                            ...
                            if (potentialSuggestions.length > 0) {
                                ...
                                summaryHtml += `<ul style="margin: 5px 0; padding-left: 20px;">`;
                                summaryHtml += potentialSuggestions.map((s) => {
                                    const scoreText = s.quality_score !== undefined ? ` (${Math.round(s.quality_score)}/100)` : '';
                    ...
                    if (result.applied_refactorings && result.applied_refactorings.length > 0) {
                        ...
                        if (result.graph_data) {
                            const nodes = ((_a = result.graph_data.nodes) === null || _a === void 0 ? void 0 : _a.length) || 0;
                            const edges = ((_b = result.graph_data.links) === null || _b === void 0 ? void 0 : _b.length) || 0;
                        ...
                        if (result.clusters && result.clusters.length > 0) {
                            ...
                            const viableClusters = result.clusters.filter((c) => { var _a; return ((_a = c.members) === null || _a === void 0 ? void 0 : _a.length) >= 3; });
                        ...
                        if (result.verification_results && result.verification_results.length > 0) {
                            const failures = result.verification_results.filter((r) => r.status !== 'PASSED_ALL');
    ...
    _updateWebview(result, output) {
        if (this._view) {
            let message = `Analysis complete but no refactorings were applied automatically.<br><br>Suggestions found: ${this._currentSuggestions.length}`;
            if (this._currentSuggestions.length === 0) {
                if (result.clusters && result.clusters.length > 0) {
                    const total = result.clusters.length;
                    const viable = result.clusters.filter((c) => c.members.length >= 2).length;
...
GenECViewProvider.viewType = 'genec.refactorView';
//# sourceMappingURL=GenECViewProvider.js.map
